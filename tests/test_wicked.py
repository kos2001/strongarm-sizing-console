...
def test_nominal_verdict_has_margins():
    r = wicked.nominal_verdict({}, {"decision_time_ps": 400, "power_uw": 400, "offset_sigma_mv": 20})
...
def test_wcd_reports_limiting_mechanism_and_positive_beta():
    r = wicked.worst_case_distance({}, {"decision_time_ps": 400, "power_uw": 400, "offset_sigma_mv": 20},
                                   n_samples=4, seed=5)
...
def test_dno_refine_enlarges_input_for_tight_offset():
    ...
    r = wicked.dno_refine(tiny, {"decision_time_ps": 400, "power_uw": 1000, "offset_sigma_mv": 5}, iterations=2)
...
def test_importance_sampling_yield_smoke():
    r = wicked.importance_sampling_yield(
        {}, {"decision_time_ps": 400, "power_uw": 400, "offset_sigma_mv": 20}, n=2, seed=9
    )
    assert r["n"] == 2
    assert 0.0 <= r["estimated_yield_pct"] <= 100.0
...
def test_parameter_screening_ranks_all_devices():
    r = wicked.parameter_screening({}, {"decision_time_ps": 400, "power_uw": 400, "offset_sigma_mv": 20}, delta=0.10)
...
def test_worst_case_corners_returns_top5():
    r = wicked.worst_case_corners({}, {"decision_time_ps": 400, "power_uw": 400, "offset_sigma_mv": 20})
    assert r["total_corners"] == 27
