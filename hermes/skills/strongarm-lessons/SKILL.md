...
## L6 · 코너 실패는 W 문제가 아니라 동작점 문제다 (2026-07-13, 코너 인지 사이징에서)

- **증상**: slow-N/-40°C/0.63V 코너에서 NMOS 스택 폭 ×8 로도 비기능 —
  "폭 보강" 조언(구 레시피)이 실측으로 반증됨.
- **원인**: 그 레짐은 서브스레숄드 — 전류가 W 에 선형, 오버드라이브에 지수.
- **규칙**: 코너 비기능엔 vcm_frac 상향/vdd 하한을 제시하라. 사이징 요청이면
  corner_aware optimize(기본)가 vcm 가용성 리프트를 자동 수행한다.
